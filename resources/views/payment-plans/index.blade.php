    ...
    <div class="top-bar">
      <div class="left-group">
        ...
        <form method="GET" action="{{ route('payment-plans.index') }}" style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
          ...
          <label><input type="checkbox" name="due_soon" value="true" {{ request('due_soon') == 'true' ? 'checked' : '' }}> Due Soon</label>
          <button type="submit" class="btn">Filter</button>
          @if(request()->hasAny(['search', 'status', 'due_soon']))
            <a href="{{ route('payment-plans.index') }}" class="btn">Clear</a>
          @endif
        </form>
      </div>
      <div class="action-buttons">
        <a href="{{ route('payment-plans.create') }}" class="btn btn-add">Add</a>
        ...
        <tbody>
          @forelse($paymentPlans as $plan)
            <tr>
              ...
              <td>{{ $plan->schedule->policy->policy_no ?? '-' }}</td>
              <td>{{ $plan->schedule->policy->client->client_name ?? '-' }}</td>
              ...
              <td>
                ...
                <a href="{{ route('payment-plans.edit', $plan->id) }}" class="btn-action">Edit</a>
                <form action="{{ route('payment-plans.destroy', $plan->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure?');">
                  @csrf
                  @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="8" style="text-align:center; padding:20px; color:#999;">No payment plans found</td>
            </tr>
          @endforelse
        </tbody>
      </table>
