...
@section('content')
@php
  $selectedColumns = session('income_columns', [
    ...
    <div class="table-responsive">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td>
              <span class="icon-expand" style="cursor:pointer;" onclick="openEditIncome({{ $inc->id }})">⤢</span>
              <form action="{{ route('incomes.destroy', $inc->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button class="btn-action btn-delete" title="Delete" style="background:none;border:none;padding:0 6px;font-size:12px;color:#dc3545;vertical-align:middle;cursor:pointer;" onclick="return confirm('Delete this income?')">
                  🗑️
                </button>
              </form>
            </td>
            @if(in_array('income_id', $selectedColumns))<td>{{ $inc->income_id }}</td>@endif
            @if(in_array('income_source', $selectedColumns))<td>{{ $inc->incomeSource ? $inc->incomeSource->name : '' }}</td>@endif
            @if(in_array('date_rcvd', $selectedColumns))<td>{{ $inc->date_rcvd }}</td>@endif
            @if(in_array('amount_received', $selectedColumns))<td>{{ $inc->amount_received }}</td>@endif
            @if(in_array('description', $selectedColumns))<td>{{ $inc->description }}</td>@endif
            @if(in_array('category', $selectedColumns))<td>{{ $inc->category }}</td>@endif
            @if(in_array('mode_of_payment', $selectedColumns))<td>{{ $inc->modeOfPayment ? $inc->modeOfPayment->name : '' }}</td>@endif
            @if(in_array('statement_no', $selectedColumns))<td>{{ $inc->statement_no }}</td>@endif
            @if(in_array('income_notes', $selectedColumns))<td>{{ $inc->income_notes }}</td>@endif
          </tr>
          @endforeach
        </tbody>
      </table>
  ...
  <div class="modal" id="incomeModal">
    <div class="modal-content">
      ...
      <form id="incomeForm" method="POST">
        @csrf
        <input type="hidden" name="_method" id="incomeFormMethod" value="POST">
        <div class="modal-body">
          <div class="form-row">
          ...
          <div class="form-row">
            ...
            <div class="form-group" style="flex:1 1 100%;">
              ...
              <textarea class="form-control" name="income_notes" id="income_notes"></textarea>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn-cancel" onclick="closeIncomeModal()">Cancel</button>
          <button type="submit" class="btn-save" id="incomeSaveBtn">Save</button>
        </div>
      </form>
    </div>
  </div>
...
function openIncomeModal(mode, inc = null) {
  ...
  if (mode === 'add') {
    ...
    document.getElementById('income_source_id').value = '';
    document.getElementById('date_rcvd').value = '';
    document.getElementById('amount_received').value = '';
    document.getElementById('description').value = '';
    document.getElementById('category').value = '';
    document.getElementById('mode_of_payment_id').value = '';
    document.getElementById('statement_no').value = '';
    document.getElementById('income_notes').value = '';
  } else if (mode === 'edit' && inc) {
    title.textContent = 'Edit Income';
    form.action = `/incomes/${inc.id}`;
    formMethod.value = 'PUT';
    currentIncomeId = inc.id;
    document.getElementById('income_source_id').value = inc.income_source_id ?? '';
    document.getElementById('date_rcvd').value = inc.date_rcvd ?? '';
    document.getElementById('amount_received').value = inc.amount_received ?? '';
    document.getElementById('description').value = inc.description ?? '';
    document.getElementById('category').value = inc.category ?? '';
    document.getElementById('mode_of_payment_id').value = inc.mode_of_payment_id ?? '';
    document.getElementById('statement_no').value = inc.statement_no ?? '';
    document.getElementById('income_notes').value = inc.income_notes ?? '';
  }
...
function openEditIncome(id) {
  fetch(`/incomes/${id}/edit`)
    .then(res => res.json())
    .then(inc => openIncomeModal('edit', inc));
}
