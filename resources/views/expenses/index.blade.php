    ...
    <div class="container-table">
      ...
      <div style="background:#fff; border:1px solid #ddd; border-radius:4px; overflow:hidden;">
        ...
        <div class="alert alert-success" id="successAlert"
          style="padding:8px 12px; margin:15px 20px; border:1px solid #c3e6cb; background:#d4edda; color:#155724;">
          {{ session('success') }}
        ...
        <div class="alert alert-danger" id="errorAlert"
          style="padding:8px 12px; margin:15px 20px; border:1px solid #f5c6cb; background:#f8d7da; color:#721c24;">
          <ul style="margin:0; padding-left:20px;">
            @foreach($errors->all() as $error)
            <li>{{ $error }}</li>
            @endforeach
          </ul>
          <button type="button" class="alert-close" onclick="document.getElementById('errorAlert').style.display='none'"
            style="float:right;background:none;border:none;font-size:16px;cursor:pointer;">×</button>
        </div>
        @endif
        ...
        <div class="table-responsive" id="tableResponsive">
          <table id="expensesTable">
              ...
              <tr>
                <th style="text-align:center;">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <path d="M12 2C8.13 2 5 5.13 5 9C5 14.25 2 16 2 16H22C22 16 19 14.25 19 9C19 5.13 15.87 2 12 2Z"
            ...
            <tbody>
              ...
              <tr
                class="{{ $expense->hasExpired ?? false ? 'has-expired' : ($expense->hasExpiring ?? false ? 'has-expiring' : '') }}">
                <td
                  class="bell-cell {{ $expense->hasExpired ?? false ? 'expired' : ($expense->hasExpiring ?? false ? 'expiring' : '') }}">
                ...
                <td data-column="amount_paid">{{ $expense->amount_paid ? number_format($expense->amount_paid, 2) : '-'
                  }}</td>
                @elseif($col == 'description')
                <td data-column="description">{{ $expense->description ?? '-' }}</td>
                @elseif($col == 'category_id')
                <td data-column="category_id">{{ $expense->expenseCategory ? $expense->expenseCategory->name : '-' }}
                </td>
                @elseif($col == 'mode_of_payment')
                <td data-column="mode_of_payment">{{ $expense->modeOfPayment ? $expense->modeOfPayment->name : '-' }}
                </td>
    ...
    <div class="footer">
      <div class="footer-left">
        <a class="btn btn-export"
          href="{{ route('expenses.export', array_merge(request()->query(), ['page' => $expenses->currentPage()])) }}">Export</a>
      ...
      <div class="paginator">
        ...
        <a class="btn-page" href="{{ $current > 1 ? page_url($base, $q, 1) : '#' }}" @if($current <=1) disabled
          @endif>&laquo;</a>
        <a class="btn-page" href="{{ $current > 1 ? page_url($base, $q, $current - 1) : '#' }}" @if($current <=1)
          disabled @endif>&lsaquo;</a>

        <span style="padding:0 8px;">Page {{ $current }} of {{ $last }}</span>

        <a class="btn-page" href="{{ $current < $last ? page_url($base, $q, $current + 1) : '#' }}" @if($current>=
          $last) disabled @endif>&rsaquo;</a>
        <a class="btn-page" href="{{ $current < $last ? page_url($base, $q, $last) : '#' }}" @if($current>= $last)
  ...
  <div class="client-page-view" id="expensePageView" style="display:none;">
    <div class="client-page-header">
      ...
      <div class="client-page-actions">
        <button class="btn btn-edit" id="editExpenseFromPageBtn"
          style="background:#f3742a; color:#fff; border:none; padding:6px 16px; border-radius:2px; cursor:pointer; display:none;">Edit</button>
        <button class="btn" id="closeExpensePageBtn" onclick="closeExpensePageView()"
          style="background:#e0e0e0; color:#000; border:none; padding:6px 16px; border-radius:2px; cursor:pointer;">Close</button>
      </div>
    </div>
    <div class="client-page-body">
      <div class="client-page-content">
        ...
        <div id="expenseDetailsPageContent" style="display:none;">
          <div style="background:#fff; border:1px solid #ddd; border-radius:4px; margin-bottom:15px; overflow:hidden;">
            <div id="expenseDetailsContent"
              style="display:grid; grid-template-columns:repeat(4, 1fr); gap:0; align-items:start; padding:12px;">
        ...
        <div id="expenseFormPageContent" style="display:none;">
          <div style="background:#fff; border:1px solid #ddd; border-radius:4px; margin-bottom:15px; overflow:hidden;">
            <div
              style="display:flex; justify-content:flex-end; align-items:center; padding:12px 15px; border-bottom:1px solid #ddd; background:#fff;">
              <div class="client-page-actions">
                <button type="button" class="btn-delete" id="expenseDeleteBtn"
                  style="display:none; background:#dc3545; color:#fff; border:none; padding:6px 16px; border-radius:2px; cursor:pointer;"
                  onclick="deleteExpense()">Delete</button>
                <button type="submit" form="expensePageForm" class="btn-save"
                  style="background:#f3742a; color:#fff; border:none; padding:6px 16px; border-radius:2px; cursor:pointer;">Save</button>
                <button type="button" class="btn" id="closeExpenseFormBtn" onclick="closeExpensePageView()"
                  style="background:#e0e0e0; color:#000; border:none; padding:6px 16px; border-radius:2px; cursor:pointer; display:none;">Close</button>
              </div>
            </div>
  ...
  <div class="modal" id="expenseModal">
    <div class="modal-content" style="max-width:800px; max-height:90vh; overflow-y:auto;">
      <div class="modal-header"
        style="display:flex; justify-content:space-between; align-items:center; padding:15px 20px; border-bottom:1px solid #ddd; background:#fff;">
        <h4 id="expenseModalTitle" style="margin:0; font-size:18px; font-weight:bold;">Add Expense</h4>
        <div style="display:flex; gap:10px;">
          <button type="submit" form="expenseForm" class="btn-save"
          ...
          <button type="button" class="btn-cancel" onclick="closeExpenseModal()"
      ...
      <form id="expenseForm" method="POST" action="{{ route('expenses.store') }}" enctype="multipart/form-data">
        ...
        <input type="file" name="receipt" id="receiptFileInput" style="display:none;"
          accept=".pdf,.jpg,.jpeg,.png,.doc,.docx">
        <div class="modal-body" style="padding:20px;">
          <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
            <div class="form-group" style="flex:1;">
              <label for="payee"
                style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Payee</label>
              <input type="text" class="form-control" name="payee" id="payee" required
                style="width:100%; padding:8px; border:1px solid #ddd; border-radius:2px; font-size:13px;">
            </div>
            <div class="form-group" style="flex:1;">
              <label for="date_paid" style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Date
                Paid</label>
              <input type="date" class="form-control" name="date_paid" id="date_paid" required
                style="width:100%; padding:8px; border:1px solid #ddd; border-radius:2px; font-size:13px;">
            </div>
            <div class="form-group" style="flex:1;">
              <label for="amount_paid" style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Amount
                Paid</label>
          ...
          <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
            <div class="form-group" style="flex:1;">
              <label for="category_id"
                style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Category</label>
            ...
            <div class="form-group" style="flex:1;">
              <label for="description"
                style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Description</label>
              <input type="text" class="form-control" name="description" id="description"
                style="width:100%; padding:8px; border:1px solid #ddd; border-radius:2px; font-size:13px;">
            </div>
            <div class="form-group" style="flex:1;">
              <label for="mode_of_payment_id"
                style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Mode Of Payment</label>
              <select class="form-control" name="mode_of_payment_id" id="mode_of_payment_id" required
                style="width:100%; padding:8px; border:1px solid #ddd; border-radius:2px; font-size:13px;">
          ...
          <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
            <div class="form-group" style="flex:1;">
              <label for="receipt_no" style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Receipt
                No.</label>
          ...
          <div class="form-row" style="display:flex; gap:15px; margin-bottom:15px;">
            <div class="form-group" style="flex:1 1 100%;">
              <label for="expense_notes"
                style="display:block; margin-bottom:5px; font-size:13px; font-weight:500;">Expense Notes</label>
              <textarea class="form-control" name="expense_notes" id="expense_notes" rows="3"></textarea>
            </div>
          </div>
          <div id="selectedReceiptPreview"
            style="margin-top:15px; padding:10px; background:#f5f5f5; border-radius:4px; display:none;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <div>
              ...
              <button type="button" onclick="removeSelectedReceipt()"
                style="background:#dc3545; color:#fff; border:none; padding:4px 10px; border-radius:2px; cursor:pointer; font-size:11px;">Remove</button>
            </div>
            <div id="selectedReceiptImagePreview" style="margin-top:10px; max-width:200px; max-height:200px;"></div>
          </div>
        </div>
        <div class="modal-footer"
          style="padding:15px 20px; border-top:1px solid #ddd; background:#fff; display:flex; justify-content:center;">
          <button type="button" class="btn-upload" onclick="openReceiptUploadModal()"
            style="background:#f3742a; color:#fff; border:none; padding:8px 20px; border-radius:2px; cursor:pointer; font-size:13px;">Upload
            Receipt</button>
          <button type="button" class="btn-delete" id="expenseDeleteBtnModal"
            style="display: none; background:#dc3545; color:#fff; border:none; padding:8px 20px; border-radius:2px; cursor:pointer; font-size:13px; margin-left:10px;"
            onclick="deleteExpense()">Delete</button>
        </div>
      </form>
    </div>
    ...
    <div class="modal-content" style="max-width:500px;">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input type="file" class="form-control" name="receipt" id="receiptFile"
            accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" onchange="handleReceiptFileSelect(event)">
          ...
          <div class="column-selection-vertical" id="columnSelection">
            ...
            <div class="column-item-vertical" draggable="true" data-column="{{ $key }}">
              ...
              <label class="column-label-wrapper">
                <input type="checkbox" class="column-checkbox" id="col_{{ $key }}" value="{{ $key }}" @if($isChecked)
                  checked @endif @if($isMandatory) disabled @endif>
